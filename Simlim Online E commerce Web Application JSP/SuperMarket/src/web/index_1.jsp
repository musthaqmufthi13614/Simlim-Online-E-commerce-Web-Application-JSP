						...
						<div role="tabpanel" class="tab-pane fade in active" id="expeditions" aria-labelledby="expeditions-tab">
							<div class="agile-tp">
								...
								<p class="w3l-ad">We've pulled together all our advertised offers into one place, so you won't miss out on a great deal.</p>
                                                    ...
                                                    <%
                                                        String custemail = session.getAttribute("uemail").toString();
                                                            try {
                                                                Connection conn;
                                                                Statement stm;
                                                                ResultSet rst;

                                                                Class.forName(TApi.DBClass.driver_url);
                                                                conn = DriverManager.getConnection(TApi.DBClass.db_url, TApi.DBClass.db_user, TApi.DBClass.db_pass);

                                                                stm = conn.createStatement();
                                                                rst = stm.executeQuery("SELECT *,count(*) as 'total_records' FROM supermarket.product");

                                                                while (rst.next()) //next read the current record and move to next,if end of record found it return false
                                                                {
                                                                    int product_id = rst.getInt("idpickup_request");
                                                                    String product_name = rst.getString("pickup_date");
                                                                    String product_category = rst.getString("pickup_time");
                                                                    String cutted_price = rst.getString("pickup_image");
                                                                    String pickup_description = rst.getString("pickup_description");
                                                                    String pickup_request_date = rst.getString("pickup_request_date");
                                                                    String status = rst.getString("status");
                                                                    double pickup_cost = rst.getDouble("pickup_cost");
                                                    %>
